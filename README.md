...
- **[`export_openvino.py`](scripts/export_openvino.py)** - Exports a model archive to OpenVINO IR (a `.xml` graph plus its `.bin` weights). Since RF-DETR does not export to OpenVINO directly, the model is first exported to ONNX and then converted with `openvino.convert_model`. Loads the model (or a user-selected checkpoint, like the other scripts) and writes the IR files to the experiment model folder by default. Exposes the same export options as `export_onnx.py`: `--opset-version`, `--batch-size`, `--dynamic-batch`, `--resolution`, and `--backbone-only`.
